          ...
          <div className="grid gap-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
            <div>
              ...
              <ul className="mt-6 space-y-3 text-sm text-ink-900/70">
                {[
                  'Белый список маршрутов: шлюз явно перечисляет, что вообще может запросить сайт бронирования — всё остальное отклоняется, не долетев до CRM',
                  'Через шлюз в обе стороны передаётся только сессионная cookie — остальные куки и заголовки обрезаются, а исходящая cookie всегда переподписывается как защищённая',
                  'Webhook-и платёжной системы принимаются только с её официальных IP-адресов — запрос с произвольного источника получает отказ',
                  'Каждый вызов через шлюз журналируется отдельно от логов CRM — виден полный след обращений с публичного сайта',
